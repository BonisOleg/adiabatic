...
# Перевірка 1: {{ на одному рядку, }} на іншому (macOS compatible)
BROKEN_VAR_TAGS=$(echo "$HTML_FILES" | xargs awk '/\{\{[^}]*$/{print FILENAME":"NR": "$0}' 2>/dev/null || echo "")
...
# Перевірка 2: {% на одному рядку, %} на іншому (macOS compatible)
BROKEN_BLOCK_TAGS=$(echo "$HTML_FILES" | xargs awk '/\{%[^%]*$/{print FILENAME":"NR": "$0}' 2>/dev/null || echo "")
