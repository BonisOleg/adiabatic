...
# Знаходимо всі CSS файли (крім reset.css)
...
if [ -z "$OVERSCROLL" ]; then
  ...
  echo "   Recommendation: Add 'body { overscroll-behavior: none; }' to reset.css"
  ((WARNING_COUNT++))
else
...
# Правило 9: touch-action: manipulation на interactive elements
echo ""
echo "👆 [Rule 9] Checking touch-action on buttons..."
TOUCH_ACTION=$(echo "$CSS_FILES" | xargs grep -c 'touch-action: manipulation' | grep -v ':0$' || echo "")
if [ -z "$TOUCH_ACTION" ]; then
  echo "⚠️  No touch-action: manipulation detected on buttons"
  echo "   Recommendation: Add 'touch-action: manipulation;' to .btn class"
  ((WARNING_COUNT++))
else
  echo "✅ touch-action: manipulation is used"
fi
