...
# Знаходимо всі HTML файли (крім node_modules)
...
# Правило 7: CSRF meta tag
echo ""
echo "🔐 [Rule 7] Checking for CSRF meta tag..."
CSRF_TAG=$(echo "$HTML_FILES" | xargs grep -l 'name="csrf-token"' || echo "")
if [ -z "$CSRF_TAG" ]; then
  echo "⚠️  CSRF meta tag not found in base.html (required for HTMX)"
  echo "   Add: <meta name=\"csrf-token\" content=\"{{ csrf_token }}\">"
  ((WARNING_COUNT++))
else
  echo "✅ CSRF meta tag found"
fi
